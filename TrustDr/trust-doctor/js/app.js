...
function bindEvents() {
  ...
  if (S.step === 3) {
    ...
    $('dl-doc')?.addEventListener('click', () => guardedDownload(() => downloadTrustDocument(S)));
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const isStripeReturn = localStorage.getItem('td_stripe_return') === '1';
  if (isStripeReturn) {
    localStorage.removeItem('td_stripe_return');
    const pendingDoc = localStorage.getItem('td_pending_doc');
    if (pendingDoc) {
      S.generatedDoc = pendingDoc;
      S.step = 3;
      S.tab  = 'doc';
      localStorage.removeItem('td_pending_doc');
    }
  }
  ...
  if (isStripeReturn) {
    /* Give the doc tab a moment to render, then show success overlay */
    setTimeout(() => showPaySuccess(), 300);
  }
});
